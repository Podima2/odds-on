...
const { Keypair, PublicKey, SystemProgram, Connection, LAMPORTS_PER_SOL } = require('../../oddson-program/node_modules/@solana/web3.js');
...
async function main() {
  ...
  const title = `Indexer smoke ${marketId.toString()}`;
  const threshold = 135;
  const timeWindowSeconds = 45;
  const now = Math.floor(Date.now() / 1000);
  const predictionCutoff = new anchor.BN(now + 8);
  ...
  const [positionAPda] = PublicKey.findProgramAddressSync(
    [Buffer.from('position'), marketPda.toBuffer(), oracle.publicKey.toBuffer()],
    PROGRAM_ID
  );
  ...
  const placeYesSignature = await program.methods
    .placePrediction({ yes: {} }, new anchor.BN(0.0003 * LAMPORTS_PER_SOL))
    .accounts({
      market: marketPda,
      position: positionAPda,
      vault: vaultPda,
      user: oracle.publicKey,
      systemProgram: SystemProgram.programId,
    })
    .rpc();
  ...
  const syncResult = await apiRequest(`/markets/${marketRow.id}/sync-chain`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
  });

  const positions = await apiRequest(`/markets/${marketRow.id}/positions`);
  const transactions = await apiRequest(`/markets/${marketRow.id}/transactions`);
    ...
    JSON.stringify(
      {
        ...
        syncSummary: syncResult.sync,
        indexedPositions: positions,
        indexedTransactions: transactions,
      },
      null,
      2
    )
